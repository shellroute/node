...
func TestBulkDisconnectJoinsErrors(t *testing.T) {
	...
	// One port fails, so bulk returns error but doesn't lose it
	if err == nil {
		// Timeout means the stuck entry blocks; that's OK for this test
	}
}
...
func TestBlockedConnectVsBulkDisconnect(t *testing.T) {
	...
	// Connect should fail (superseded by bulk)
	if connectErr == nil || !errors.Is(connectErr, ErrLifecycleBusy) {
		// Also acceptable: ErrAlreadyExists if manager returned that
	}
	_ = bulkErr
